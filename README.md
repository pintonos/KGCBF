...
The Knowledge Graph Curation Benchmark Framework (KGCBF) is a framework for evaluating error detection- and correction approaches in knowledge graphs. It is split into a corruption module and an evaluation module, which are accessed using a command line interface.  
...
| --shacl      | If set, SHACL shapes are automatically inferred for domain and range violations based on the output graph's TBox. |
...
errors:
  ...
  syntactic:
    ...
    PropertyAssertionError: 0.1
```
For each error, a value between 0 and 1 can be set. This refers to the percent of triples in the input graph that should be corrupted with this error.
Depending on the error, this value may be approximated. In these cases, the true number of errors may be slightly lower. However, 
it will never exceed the set parameter. The final report will give more precise information about the actual number of errors introduced.
...
We ensure that the altered triples still conform to the Turtle and RDF syntax.
...
- `InstanceIdentifierError`: The identifier of an instance does not conform to the RDF syntax.
- `TypeError`: The type of a literal does not conform to the RDF syntax.
- `PropertyNameError`: The name of a property does not conform to the RDF syntax.
...
**Subgraph**: The corruption module can be configured to only use a subgraph of the input graph. This is useful if the
input graph is too large to be corrupted in a reasonable amount of time. The size of the subgraph is defined via the `--subgraph` parameter,
 and is set between 0 and 1. This number refers to the fraction of the original graph that should be used for corruption.
...
installed on the machine, so tha the file `bin/RDFDoctor.jar` can be executed. The `validatrr` approach uses a dockerized 
...
corrupt_instance_id: # define types of error which the approach can detect (see corruption module)
...
corrupt_property:
  patterns:
    - "Error[1-9]+.*\n.*$corrupt.predicate$" # use the patterns array anyway, even for a single pattern
```
